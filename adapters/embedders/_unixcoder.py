...
class UniXcoder(nn.Module):
    ...
    def tokenize(
        self, inputs, mode="<encoder-only>", max_length=512, padding=False
    ):
        """
    ...
    def forward(self, source_ids):
        ...
        sentence_embeddings = (token_embeddings * mask.unsqueeze(-1)).sum(
            1
    ...
    def generate(
        ...
        for i in range(source_ids.shape[0]):
            context = [
                [
                    x[i : i + 1, :, : source_len[i]].repeat(beam_size, 1, 1, 1)
                    for x in y
                ]
